Glide = Glide or {}
Glide.LuaRoot = "zcity_ce/gamemode/vendor/glide/runtime/"
Glide.ThemeRoot = "zcity_ce/gamemode/vendor/glide/styled_theme/"

function Glide.GetLuaPath( path )
    return Glide.LuaRoot .. ( path or "" )
end

function Glide.GetThemePath( path )
    return Glide.ThemeRoot .. ( path or "" )
end
...
do
    ...
    function Glide.ShouldIncludeLuaFile( path )
        if CLIENT and path == Glide.GetLuaPath( "client/engine_stream_webaudio.lua" ) and not IsWebAudioAvailable() then
...
if SERVER then
    ...
    -- Shared files
    Glide.IncludeDir( Glide.GetLuaPath(), true, true )

    -- Server-only files
    Glide.IncludeDir( Glide.GetLuaPath( "server/" ), true, false )

    -- Client-only files
    AddCSLuaFile( Glide.GetThemePath( "styled_theme.lua" ) )
    AddCSLuaFile( Glide.GetThemePath( "styled_theme_tabbed_frame.lua" ) )
    AddCSLuaFile( Glide.GetThemePath( "styled_theme_file_browser.lua" ) )

    Glide.IncludeDir( Glide.GetLuaPath( "client/" ), false, true )
    Glide.IncludeDir( Glide.GetLuaPath( "client/vgui/" ), false, true )
end

if CLIENT then
    -- Shared files
    Glide.IncludeDir( Glide.GetLuaPath(), true, false )

    -- UI theme library
    include( Glide.GetThemePath( "styled_theme.lua" ) )
    include( Glide.GetThemePath( "styled_theme_tabbed_frame.lua" ) )
    include( Glide.GetThemePath( "styled_theme_file_browser.lua" ) )
    ...
    -- Client-only files
    Glide.IncludeDir( Glide.GetLuaPath( "client/" ), true, false )
    Glide.IncludeDir( Glide.GetLuaPath( "client/vgui/" ), true, false )
end

-- Automatically include files under
-- `gamemode/vendor/glide/runtime/autoload/`, on both server and client.
Glide.IncludeDir( Glide.GetLuaPath( "autoload/" ), true, true )
