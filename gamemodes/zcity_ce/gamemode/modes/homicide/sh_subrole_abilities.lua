...
function MODE.GetPlayerTraceToOtherVictim(ply, victim, dist)
	if(IsValid(victim))then
		...
		if(bone_id)then
			...
			if(bone_matrix)then
				local pos, _ = bone_matrix:GetTranslation(), bone_matrix:GetAngles()
...
function MODE.ContinueBreakingOtherNeck(ply)
	...
	local aim_ent, other_ply, _ = MODE.GetPlayerTraceToOtherVictim(ply, victim)
...
function MODE.ContinueDisarmingOther(ply)
	...
	local aim_ent, other_ply, _ = MODE.GetPlayerTraceToOtherVictim(ply, victim, MODE.DisarmReach)
