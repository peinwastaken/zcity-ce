...
function zb.GetMapPoints( pointGroup, forceupdatepoints ) -- Load points into game memory... The client will have roughly the same function.
...
function zb.CreateMapPoint( pointGroup, pointData, needsave ) -- Create a point on the map, and should it be saved?
    if not zb.CreateMapDir() then PrintMessage( HUD_PRINTTALK, "sv_points.lua: map folder dosen't exists?" ) return false end
    if not zb.Points[pointGroup] then PrintMessage( HUD_PRINTTALK, "sv_points.lua: point group " .. "\"" .. pointGroup .. "\"" .. " doesn't exist." ) return false end
...
function zb.RemoveMapPoint( pointGroup, pointNum, needsave, removeall ) -- Create a point on the map, and should it be saved?
    if not zb.CreateMapDir() then PrintMessage( HUD_PRINTTALK, "sv_points.lua: map folder dosen't exists?" ) return false end
    if not zb.Points[pointGroup] then PrintMessage( HUD_PRINTTALK, "sv_points.lua: point group " .. "\"" .. pointGroup .. "\"" .. " doesn't exist." ) return false end
...
function zb.SetMapPoint( pointGroup, pointNum, pointData, needsave ) -- Create a point on the map, and should it be saved?
