...
function SWEP:GetZoomPos(recoilZoomPos, view, eyePos)
	...
	if eyePos then
		if self:HasAttachment("sight","optic") then
			...
			local _, _, dist = util.DistanceToLine(posZoom, posZoom + (self:GetOwner():GetAimVector()), eyePos)
			dist = dist - 1
...
function SWEP:Camera(eyePos, eyeAng, view, vellen, ply)
	...
	local zoom = self:IsZoom() and (IsValid(ply.FakeRagdoll) or ((self.lerpaddcloseanim * self.closeanimdis) < 3)) and (self:GetNetVar("shootgunReload", 0) < CurTime())
	...
	fovlerp = Lerp(0.01, fovlerp, -suicVal + self.shot * 5)
...
hook.Add("HUDPaint", "homigrad-test-att", function()
	...
	if wep.EjectAng then
		local _, ejectAng = LocalToWorld(vecZero,wep.EjectAng,vecZero,ang)
		ang = ejectAng
	end
