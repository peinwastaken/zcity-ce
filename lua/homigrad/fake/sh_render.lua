...
--\\ DrawPlayerRagdoll
	...
	local zc_firstperson_ragdoll = ConVarExists("zc_firstperson_ragdoll") and GetConVar("zc_firstperson_ragdoll") or CreateConVar("zc_firstperson_ragdoll", "0", FCVAR_ARCHIVE, "Toggle first-person ragdoll camera view", 0, 1) --!! unused??
	local zc_firstperson_death = ConVarExists("zc_firstperson_death") and GetConVar("zc_firstperson_death") or CreateClientConVar("zc_firstperson_death", "0", true, false, "Toggle first-person death camera view", 0, 1)
	...
	CreateClientConVar("zc_deathfadeout", "1", true, true, "Toggle screen fade and sound mute on death", 0, 1)
	...
	function DrawPlayerRagdoll(ent, ply) --// actually not only ragdoll render but player too
		...
		local wawanted = (GetViewEntity() != ply) and !fountains[ent] and (!(!lply:Alive() and lply:GetNWEntity("spect") == ply and viewmode == 1) and !(zc_firstperson_death:GetBool() and follow == ent)) and vector_full or vector_small
		...
		--if !current:IsEqualTol(wawanted, 0.01) then
			...
			if !(Glide and Glide.Camera and !Glide.Camera.isInFirstPerson and lply == ply and lply:InVehicle() and zc_no_camera_in_cars:GetBool()) then
				if (!zc_thirdperson:GetBool() and !zc_gopro:GetBool() and (ent == ply or (!zc_ragdollcombat:GetBool() or zc_firstperson_ragdoll:GetBool()))) or (zc_firstperson_death:GetBool() and follow == ent) then
