...
--\\
	...
	--=\\
		...
		vocal_funcs["tri"] = function(args, val, current_frequency, work_table)
			local iters, trebble, len, _, mul = math.Round(args.Pitch), args.Trebble or 1, args.Length or 1, args.SineDiv or 1, (args.Mul or -1)
			local sample_val = 0
		...
		local function generate_func(sample_index)
			...
			if(freq_info_next)then
				...
				--\\
					...
					for arg_id, _ in pairs(freq_info[5]) do
